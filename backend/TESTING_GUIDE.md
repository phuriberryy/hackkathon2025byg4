...
1. **Import Collection**
   ...
   - Select `ShareCycle_API.postman_collection.json`
...
**Testing Scenario:**
```
User A posts Item (bike)
    ↓
User B creates exchange request (skateboard offered)
    ↓
User A (owner) accepts exchange
    ↓
User B (requester) accepts exchange
    ↓
Chat created automatically
    ↓
Both scan QR code to confirm exchange
    ↓
Exchange marked completed
    ↓
CO2 saved calculated and sent to both
```
...
| `ShareCycle_API.postman_collection.json` | Postman collection |
| `scripts/run-tests.js` | Automated test runner |
| `scripts/test-db-connection.js` | Database connectivity test |
| `scripts/test-email.js` | Email/SMTP test |
...
2. **Import Postman Collection**: Get from `ShareCycle_API.postman_collection.json`
